...
**현재 게임 습관:**
- 퍼즐&드래곤즈를 3년 이상 플레이했고 드래그 젬 매칭 조작에 이미 익숙. 최근 콘텐츠 반복에 지루함을 느껴 비슷한 시스템의 새 게임을 탐색 중
- 출퇴근 지하철 10~15분, 점심 짬에 한두 판. "딱 한 판만 더"를 유발하는 짧고 긴장감 있는 전투를 선호
- 퍼즐&드래곤즈 대비 "실시간 긴장감"이 추가된 게임을 원함 — 적 게이지가 차기 전에 처치해야 하는 압박감

**이 게임을 플레이하는 맥락:**
- 퍼즐&드래곤즈 경험 덕분에 젬 드래그 조작은 튜토리얼 없이 즉시 파악 가능
- 파티 편성→전투→결과 확인→재도전 사이클이 10분 안에 완결되어야 함
- 적 공격 게이지 실시간 표시로 "지금 어떤 속성을 써야 하나"를 매 순간 판단하는 전술적 쾌감을 즐김
- 속성 상성 조합을 스스로 발견하는 "아 이렇게 하면 되는구나" 순간을 퍼즐&드래곤즈보다 더 빠르게 경험하고 싶음
...
### 핵심 강점
- 기획 품질 우수: 헥사고날 퍼즐 + 실시간 적 공격 게이지 조합은 퍼즐&드래곤즈 대비 실시간 긴장감을 추가하는 명확한 차별화 포인트
- 레퍼런스 분석(퍼즐&드래곤즈, Hearthstone, Into the Breach)이 구체적이고 적용 교훈이 설계에 반영됨
- Phaser.js 기반 구현 50% 완성, 핵심 드래그 루프는 작동 중

### 치명적 약점
- 퍼즐&드래곤즈가 글로벌 1억 다운로드 이상 — 직접 경쟁에서 차별화 훅이 "실시간 긴장감" 하나뿐으로 약함
- 적 공격 게이지 실시간 표시가 실제로 퍼즐&드래곤즈 대비 긴장감 차별점이 되는지 플레이 테스트 미검증
- 빌드/실행 안정성 40%로 포트폴리오 내 하위권 — 출시까지 거리가 멀다

### 보류 조건
보류 해제 조건 충족 시 재판단:
1. 퍼즐&드래곤즈 대비 실시간 긴장감 훅이 실제로 차별화되는지 프로토타입 10인 검증 (재시작률 60% 이상 기준)
2. 빌드/실행 안정성 70% 이상으로 개선
위 조건 미충족 시 ElementalCommand보다 구현 완성도가 높은 출시 추진 게임에 리소스 우선 배분.
...
### 보류 이유
- 퍼즐&드래곤즈의 직접 아류 — 헥사 보드+실시간 게이지만으로 차별화 불충분
- 빌드 안정성 40%로 최하위권
- 실시간 퍼즐은 입력·타이밍 버그 취약

### 재착수 조건
- [ ] 퍼즈드래와 차별화되는 핵심 메커닉 1개 추가 및 검증
- [ ] 빌드 안정성 70% 이상 달성
- [ ] 실시간 입력 버그 0개 기준 QA 통과
